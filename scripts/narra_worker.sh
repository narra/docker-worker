...
if [ $DEBUG == "true" ]; then
    exec /sbin/setuser app /usr/bin/bundle exec rdebug-ide --host 0.0.0.0 --port 1234 --dispatcher-port 26162 -- /usr/local/rvm/gems/default/bin/sidekiq -C config/sidekiq_${NARRA_WORKER_TYPE}.yml
else
